...
ms.locfileid: "129072467"
---
# <a name="bulk-operations-for-co-sell-opportunities-using-comma-separated-value-csv-files"></a>Operações em massa para oportunidades de venda conjunta com ficheiros de valores separados por vírgula (CSV)
...
- Linha de Endereço do Cliente 2
- Cidade do Cliente
- Estado do Cliente
- Código Postal do Cliente
- País do Cliente
- ID do cliente D-U-N-S
- Consentimento para partilhar contacto cliente/parceiro
- ID do MPN

### <a name="fields-that-will-become-read-only-after-you-first-share-them"></a>Campos que se tornarão lidos apenas depois de os partilhares pela primeira vez

- **O Primeiro Nome do Contacto com o Cliente** só irá ler depois de submeter os detalhes para co-venda de criação ou upgrade
- **O Nome Do Último Nome do Contacto do Cliente** só entrará em leitura depois de submeter os detalhes para co-venda de criação ou upgrade de referência
- **O número de Telefone contacto com o cliente** só irá ser lido depois de submeter os detalhes para co-venda de criação ou atualização de referência
- **O Endereço de E-mail do Cliente** entrará em leitura apenas depois de submeter os detalhes para co-venda de criação ou atualização de referência
- **Ajuda da Microsoft necessária?** vai transformar a leitura-only depois de marcá-lo como Sim
- **Que ajuda específica da Microsoft?** irá transformar apenas a leitura depois de introduzir um número no formato especificado
- **Partilhe com a equipa de vendas da Microsoft** que só irá ler depois de o marcar como Sim
- **As notas para a Microsoft** só irão ser lidas depois de submeter as notas para co-venda de criação ou upgrade

Os detalhes de todas as colunas com os seus dados e valores de amostra estão na tabela abaixo.
...
ID de noivado|Yes|O ID de noivado é gerado pelo sistema de referências do Microsoft Partner Center. Não é necessário para a nova criação de referência. Pode utilizar a identificação de noivado existente se estiver a atualizar um registo.|f7eaae47-0b84-4ac4-b4ea-5b2587d42cee
ID de referência|Yes|O ID de referência é gerado pelo sistema de referências do Microsoft Partner Center. Não é necessário para a nova criação de referência. Preencha-o com a identificação de encaminhamento se estiver a atualizar um registo existente.|ebacdkdc-0b84-4ac4-b4ea-5b2587d42cee
...
Linha de Endereço do Cliente 1|Yes|Linha de endereço 1 da empresa cliente. |Um Caminho de Contoso
...
ID do cliente D-U-N-S|No|Tente obter a identificação duns da organização do cliente. Isto ajudará a combinar mais rapidamente a organização do cliente do lado da Microsoft, o que ajuda a uma atribuição mais rápida do vendedor. Você pode obter DUNS ID FREE a partir da [página de aparência de números D-U-N-S](https://www.dnb.com/duns-number/lookup.html).|81466849
Contato com o cliente Primeiro Nome|Depende|O primeiro nome só é obrigatório se precisar de ajuda da Microsoft. O primeiro nome do contacto principal da organização de clientes que trabalha neste negócio.|John
Contato com o cliente Nome do Último Nome|Depende|O apelido só é obrigatório se precisar da ajuda da Microsoft. Último nome do contacto principal da organização de clientes que trabalha neste negócio.|Cliente
...
Serviços de Anexação|No|Isto é para indicar se os serviços Azure devem ser agregados com a solução IoT na implementação
Microsoft MSX ID|No|ID de um negócio no sistema Microsoft MSX, disponível apenas para co-vender
ID de acordo com o PSC emigrou|No|ID do negócio do PSC, apenas disponível para um negócio migrado do PSC para PC
ID do MPN|No|ID MPN da organização para a qual estão a ser criadas as oportunidades de co-venda
Solução 1|Yes|ID de solução (obrigatório), a moeda (opcional) na qual o valor do negócio é inserido. Pode encontrar os [códigos de moeda,](https://en.wikipedia.org/wiki/ISO_4217)preço do SKU (opcional) e quantidade do SKU (opcional)  |SOL-1234-PQRS, USD, 10,100
