...
Para adquirir uma subscrição SaaS, precisa de uma conta de utilizador Azure com acesso a uma subscrição Azure apropriada. Esta subscrição será usada para faturação, bem como para compartimentação dos seus recursos de nuvem adquiridos. Para saber mais sobre as subscrições do Azure, consulte [criar uma subscrição adicional do Azure.](/azure/cost-management-billing/manage/create-subscription)
...
Cada oferta saas terá um ou mais planos. Cada oferta tem um modelo de preços associado: taxa fixa ou por utilizador. Cada preço do plano é uma taxa recorrente, que pode ser zero dólares (quaisquer preços cotados são apenas para fins e não se destinam a refletir os custos reais). Esta taxa é forfeso ou preço por utilizador. Tipos de planos disponíveis:
...
- **Medidores personalizados** – juntamente com as taxas recorrentes, um plano de taxa fixa também pode incluir dimensões personalizadas opcionais para o uso de overage não incluído na taxa fixa. Cada dimensão representa uma unidade de faturação. Este é um custo variável que muda de acordo com o uso de unidades medidas, tais como: largura de banda, bilhetes ou e-mail processado. Será cobrado de acordo com o seu consumo destas dimensões mensalmente. Por favor, note que o consumo excessivo só começa quando utiliza todas as unidades de medição incluídas na taxa fixa.
...
2. Como parte do processo de compra, você será redirecionado para o separador **Básicos** onde você precisa:

    1. Defina qual *subscrição* gostaria de utilizar para faturação. A assinatura Azure que utiliza deve ter um método de compra válido definido para o mesmo. Deve ter o nível de permissão certo ou ter um grupo de recursos sob essa subscrição com o nível certo de permissões. Além disso, o seu país de faturação deve ser um país onde a oferta está disponível para compra. As assinaturas Azure sem um método de pagamento válido (por exemplo, uma subscrição MSDN) só podem ser usadas para comprar planos gratuitos.
        > [!NOTE]
        > As seguintes assinaturas Azure não podem ser usadas para comprar SaaS: Estudante, Visual Studio Enterprise ou Crédito Gratuito.
    ...
    1. Em **Plano,** verá o plano que selecionou na página detalhada do produto (PDP). Se não fez uma seleção ativa no PDP, verá o plano padrão. Pode alterar a sua seleção selecionando o link **do plano Change.** Selecione o termo de faturação relevante e, em seguida, escolha outro plano. Poderá alterar o plano após a compra, se a editora o apoiar. No entanto, não poderá alterar o prazo de mensal para anual ou de anual para mensal.
    1. Nos casos em que o modelo de preços é *por utilizador,* poderá ser-lhe exigido que especifique o número de *Utilizadores*. O preço que vê vai mudar com base na subscrição, plano e termo que selecionou.
...
4. Continue a **Rever + Subscreva** para analisar a oferta e planear detalhes.
    1. Rever *Termos de utilização,* *Alterações* e Política de *Privacidade* da editora e também para o Azure Marketplace.
    1. Pode ser-lhe pedido que adicione os seus dados de contacto.
    1. *Reveja detalhes básicos* e *tags.*
...
Se não concluir o processo de configuração dentro de *30 dias,* esta subscrição do SaaS será automaticamente *eliminada*. A faturação começará depois de a sua conta estar configurada no site da editora.

### <a name="common-error-messages-and-solutions"></a>Mensagens e soluções de erro comuns

Estas são mensagens de erro comuns que poderá encontrar durante o processo, juntamente com soluções comuns:
...
- A compra falhou porque não conseguimos encontrar um cartão de crédito válido, nem um método de pagamento associado à sua subscrição Azure.
  - Utilize uma subscrição Azure diferente ou adicione o cartão de crédito atual ou o método de pagamento atual para esta subscrição e redaveja.
...
- O *nome do plano selecionado* pelo editor de *oferta* não está disponível para compra devido a configurações de mercado privados feitas pelo administrador de TI do seu inquilino. 
  - Contacte o seu administrador de TI.
...
- A compra falhou porque não pudemos verificar a sua assinatura de um acordo legal.
  - Reda o trabalho. Se o erro persistir, tente fazer a compra utilizando diferentes subscrições ou suporte de contacto da Azure.

- A compra da *ofertaID* pela *editora editorID* falhou. Esta oferta não está disponível para compra.
  - Tente novamente mais tarde. Se, passado uma hora, continuar a receber esta mensagem de erro, contacte o suporte.  

- A aquisição do *plano de ofertaID* da *ofertaID* pela *editora editoraID* falhou. Este plano não está atualmente disponível para compra.
  - Tente novamente mais tarde. Se, passado uma hora, continuar a receber esta mensagem de erro, contacte o suporte.

- O *endereço de e-mail* do cliente com *o objeto id ObjectID* não tem autorização para executar *a implementação implementação DevalidationAction* over scope *ResourceGroup; O DeploymentScope* ou o âmbito de aplicação é inválido.  
...
- A subscrição utilizada para esta compra não permite compras no Marketplace.  
  - Por favor, use uma subscrição diferente ou peça ao seu administrador para alterar a definição para esta subscrição e relemissa.
